...
import {
  ...
  LineChart, SquareMenu, ConciergeBell, History, Scale, MonitorUp,
} from "lucide-react";
...
const STORE_NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: "레시피 정보", href: "/recipes",
        icon: <ChefHat className="h-4 w-4" />,
        mobileIcon: <ChefHat className="h-5 w-5" />,
        roles: ["master", "admin", "manager", "staff"],
      },
      {
...
const POS_NAV_GROUP: NavGroup = {
  ...
  items: [
    ...
    {
      label: "주방 KDS", href: "/pos/kds",
      icon: <MonitorUp className="h-4 w-4" />,
      mobileIcon: <MonitorUp className="h-5 w-5" />,
      roles: ["master", "admin", "manager", "staff"],
    },
    {
      label: "주문 이력", href: "/pos/orders",
    ...
    {
      label: "일일 대조", href: "/pos/reconciliation",
      icon: <Scale className="h-4 w-4" />,
      mobileIcon: <Scale className="h-5 w-5" />,
      roles: ["master", "admin", "manager"],
    },
  ],
};
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden selection:bg-primary/20">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        >
          <div className="pb-40 lg:pb-8 pt-4 lg:pt-6 min-h-full" style={isStaff && zoomLevel > 100 ? { zoom: zoomLevel / 100 } : undefined}>
