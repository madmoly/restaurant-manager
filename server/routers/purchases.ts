...
import { db } from "../db";
import { purchaseOrders, purchaseOrderItems, counterparties, purchaseOrdersV2, dailyExpenses } from "../../drizzle/schema";
  ...
  monthlyTotal: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [v2Row] = await db
        .select({ total: sql<string>`COALESCE(SUM(CAST(${purchaseOrdersV2.totalAmount} AS DECIMAL(14,2))), 0)` })
      ...
      const total = Number(v1Row?.total ?? 0) + Number(v2Row?.total ?? 0) + Number(expRow?.total ?? 0);
