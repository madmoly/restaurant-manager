...
> 마지막 갱신: 2026-04-27 (세션 핸드오프 프로토콜 추가)
...
**전역지침 "작업 게이트"와의 관계** (명시적 예외):
- 전역지침 §작업 게이트(30분+ 작업 시 TASK_*.md 의무)는 **Cowork 진입 시점**에 적용된다.
- Code로 핸드오프된 이후의 실행 단계에서는 본 문서 §3 "자율 실행 규칙"이 작업 게이트를 대체한다.
- 따라서 Code 세션 안에서는 30분+ 작업이라도 TASK_*.md 별도 작성 의무 없음. 단, §3 보고 빈도("30분+ 시 1회 중간 요약")는 유지.
- 새 기능·구조 변경의 **설계 단계**에서 30분+ 소요가 예상되면 Cowork 측에서 TASK_*.md 작성 후 Code로 넘긴다(원칙).

**전역지침 "세션 핸드오프"와의 관계** (Code 측 의무):
- 활성 TASK_*.md가 있는 작업이면 Code 새 세션 시작 시 그 진행 로그 마지막 줄을 먼저 읽는다.
- 작업 완료 시(보고 직전 또는 push 직전) 진행 로그에 한 줄 append: `YYYY-MM-DD Code(SHA) — 한 일 / 미해결`. 이는 파일 갱신이며 사용자 화면 보고가 아니므로 §3 "중간 보고 금지" 규칙과 무관.
- TASK_*.md가 없는 작업(버그 수정·텍스트 편집·자율 실행 단발)은 본 규칙 비활성.

## 3. 자율 실행 규칙

**원칙**: 지시받은 작업은 **끝까지 자율 실행**. 중간 단계마다 허락 묻지 말 것. 최종 결과를 한 번에 요약 보고. 막히면 대안까지 자율 시도하고, 그래도 안 되면 그때 보고.

### 자율 실행 허용 (사전 확인·중간 보고 금지)
- 파일 읽기/쓰기/수정/삭제 (프로젝트 루트 범위 내)
- Bash, grep, sed, find, 파이프라인 조합
- `git add`, `git commit`, `git stash`, `git branch`, 로컬 체크아웃
- `pnpm run build`, test, lint, type-check
- DB READ-ONLY SELECT (Railway prod 포함)
- 가설 검증·코드 분석·리팩터 초안 작성
- 에러 재현·로그 수집·의존성 조사

### 정지 조건 — 아래 7개 상황에서만 멈추고 승인 대기
1. **`git push origin main` 직전** — §4 5항 요약 보고 후 승인 대기
2. `git push --force`, 브랜치 강제 삭제(`-D`), `git commit --amend`, 이력 재작성
3. **DB WRITE** (UPDATE/DELETE/DROP/TRUNCATE/ALTER/INSERT) 실행 직전 — Railway prod MySQL 대상
4. Railway 환경변수 변경, 서비스 재시작, 볼륨 삭제, 플랜 변경
5. `rm -rf` 프로젝트 루트 이상 범위, 추적 중인 파일 대량 삭제
6. 외부 유료 API 대량 호출 (Anthropic OCR 벌크 등, >10회 기준)
7. **근본 분기 판단** — 수정 범위가 급격히 확장되거나 CLAUDE.md 원칙과 충돌 가능한 선택지가 등장한 경우

### 보고 빈도
- 정지 조건 외에는 **중간 진행 보고 금지**. 최종 결과만 한 번.
- 예외: 작업이 30분 이상 걸릴 경우, 한 번만 중간 상태 요약 허용.
- 보고 형식은 건조·사실 중심. 서두 인사·결과 반복·자기 확인 질문 금지.

### 실패 처리
- 1차 시도 실패 → 원인 분석 후 대안 자율 시도
- 2~3차까지 실패 → 그때 막힌 지점·시도 이력·남은 옵션을 정리해 보고
- 실패 보고에 "어떻게 할까요?"보다 **"A/B/C 중 고르세요"** 형식 선호
...
| 볼륨 | 50 GB/서비스, 10개/프로젝트 | $0.15/GB 월 |
| 임시 스토리지 | 100 GB | 배포 시 초기화 |
| 네트워크 Egress | — | $0.05/GB |
| 이미지 보존 | 72시간 | 롤백용 |

**주의**: DB(MySQL)도 볼륨 기반 → 50GB 한도. 파일 업로드는 임시 스토리지라 배포 시 초기화 → 영구 저장 필요 시 외부 스토리지 검토.
