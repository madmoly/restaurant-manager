...
- 2026-07-29 Code(post-v4) — **P0~P7 전체 구현 + 배포 게이트 안정성 확보(연속 6/6 + 회귀)**. P0 재확인: grandTotal 오독 미발생(590,006/300,000), 빈 셀 <td></td> 보존, 백합 exempt. **v4 핵심 구현**: detectUnitPriceBasis 신설(과세 행 다수결, tol=max(2,qp×0.5%), v=0 제외) → validateAndEnrichItems/reconcileTotals에 basis 인자 관통, 면세 안전망 basis==="gross" 게이트(리스크#2 — 유닛으로 net 과세행 taxable 유지 확인), net lineTotal=공급가+부가세 교정(리스크#3), basis unknown 시 (a)(b) ×1.1 보정 건너뜀(리스크#4), 프롬프트 전면 판별형 전환(STEP 2.5 ⓐⓑ 판별, Vision ×1.1 단정 제거, [A] 예시 완화, 합계행 당기/채권 축 구분), 프로파일 priceBasis+priceBasisCount 축적(히스테리시스)·폴백(basisSource=profile)·힌트 주입, totals에 basis/basisSource/basisEvidence + UI 표시. **안정화 과정에서 발견·수정 3건**: (i) 판별형 프롬프트가 문서 값 재계산 출력 유발 → "관계식은 검증용, 재계산 금지" 명시로 해소 (ii) 모델이 합계행 전체를 채권 축으로 오분류해 summary 통째 누락(0344 라벨 없는 별도 테이블) → 값 단위 축 구분 프롬프트 + **extractTotalsFromTables 서버 폴백**(한 행 연속 세 숫자 a=b+c 삼중조, 합계행 키워드 게이트+역순 탐색 — 품목행 오탐은 키워드 게이트로 차단, 채권 값은 관계식 불만족으로 자기필터) (iii) gross 서명 대칭 안전망(supply≈ceil(qp/1.1) → taxable 복원, 모델 부가세 누락 런 방어). **게이트**: IMG_0344/0346 각 3회 연속 전 항목(9/7항목, Σlt/Σs/Σv, 과세/면세 수, basis=gross, match, 병합) 통과 — 0344는 매 런 표추출 폴백으로 안정, 회귀 test-ocr(2항목·전행 unknown·no_doc_total·폴백 미발화), 음성(배추 제거 → mismatch −29,900 + 축검증 경고), basis 유닛 5종·basis별 세구분 유닛 4종·(a0)·축검증 전부 통과, build 통과. 미해결: net 유형 실물 전표 미확보(유닛으로만 검증 — 배포 후 [OCR] basis= 로그 수집), extract-statement 경로는 basis 미적용(별도 taxType 로직 사용 중).
- 2026-07-29 Code(ee53759→1f7a098→02fc05e→v4보완) — **extractTotalsFromTables 보완 3건 완료**. ① 첫 매치 반환 → 행 내 유효 삼중조 전부 수집 후 **최소 a 채택**(누적 잔액 ≥ 당기 거래; 순서 반전 행 [합계 17,230,551|이월 16,640,545|590,006|557,398|32,608]에서 590,006 채택 유닛 확인, 행 단위 결정·행 간 후보 수집 없음) ② OcrTotals.totalsSource("model"|"table_fallback") 추가, 호출부 2곳에서 기록·reconcile에 전달 ③ 폴백 앵커 시 (a)부가세 미합산·(b)이중합산 억제(±10% 조용한 변경 차단, (a0)/(c)/(d)는 산술 자기완결이라 유지) + 클라 저장 게이트 확장(mismatch ∥ auto_fixed×table_fallback 시 confirm) + 노란 배너에 "표에서 자동 복원" 문구(status는 auto_fixed 유지 — mismatch 승격 안 함). **완료 조건 전부 통과**: 유닛 6종(순서반전/기존순서 불변/단일 삼중조/폴백 (a) 억제/model (a) 회귀/폴백에서 (c) 유지), 실물 게이트 8/8(IMG_0344 ×3 — model 앵커 2회·폴백 앵커 1회 모두 match, IMG_0346 ×3, test-ocr 회귀, 음성 −29,900), build 통과. 미해결: net 실물 미확보(합의된 대로 배포 후 [OCR] basis= 로그 수집).
